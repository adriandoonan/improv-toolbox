...
type FilterMode = "equals" | "includes";
...
export interface ResourceColumnConfig {
  ...
  fallback?: (card: HTMLLIElement) => string;
}
...
interface ResourceRow extends Record<string, string> {
  id: string;
  favorite: string;
  cardElement: HTMLLIElement;
  favoriteRoot: Nullable<HTMLElement>;
  favoriteButton: Nullable<HTMLButtonElement>;
  favoriteKey: string;
}
...
export function setupResourceTable(options: ResourceTableOptions): () => void {
  ...
  const cards = Array.from(list.querySelectorAll<HTMLLIElement>(".resource-card"));
  if (!cards.length) {
    logDebug("No resource cards found", { listSelector });
    return () => {};
  }

  const columns: ResourceColumnConfig[] = [
    ...columnConfig,
    {
      key: "favorite",
      filter: "equals",
  ...
  const columnDefs: ColumnDef<ResourceRow, unknown>[] = columns.map(
    ({ key, filter, enableGlobalFilter = true }) =>
  ...
  const table = createTable<ResourceRow>({
    data: resourceRows,
    columns: columnDefs,
    state: tableState,
