...
- **2025-10-20:** Shifted interactive scripts to build-time-hashed `.client.ts` bundles (loaded via `?url`) and bumped the service worker cache (`toolbox-v4`) so PWAs fetch fresh bundles without MIME errors during deploys.
